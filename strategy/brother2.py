...
class TradeStrategy(BaseModule):
    ...
    def async_query(self, query_type: str, **kwargs):
        ...
        self.raw_redis.publish(self.__request_format.format('ReqQry' + query_type), orjson.dumps(kwargs))
    ...
    async def SubscribeMarketData(self, inst_ids: list):
        ...
        try:
            ...
            task = asyncio.create_task(self.query_reader(sub_client))
            self.raw_redis.publish(self.__request_format.format('SubscribeMarketData'), orjson.dumps(inst_ids))
            await asyncio.wait_for(task, HANDLER_TIME_OUT)
            await sub_client.punsubscribe()
            await sub_client.close()
            return task.result()
    ...
    async def UnSubscribeMarketData(self, inst_ids: list):
        ...
        try:
            ...
            await sub_client.psubscribe(channel_rsp_dat, channel_rsp_err)
            task = asyncio.create_task(self.query_reader(sub_client))
            self.raw_redis.publish(self.__request_format.format('UnSubscribeMarketData'), orjson.dumps(inst_ids))
            await asyncio.wait_for(task, HANDLER_TIME_OUT)
            await sub_client.punsubscribe()
            await sub_client.close()
            return task.result()
    ...
    def ReqOrderInsert(self, sig: Signal):
        try:
            ...
            self.raw_redis.publish(self.__request_format.format('ReqOrderInsert'), orjson.dumps(param_dict))
        except Exception as e:
    ...
    async def cancel_order(self, order: dict):
        ...
        try:
            ...
            task = asyncio.create_task(self.query_reader(sub_client))
            self.raw_redis.publish(self.__request_format.format('ReqOrderAction'), orjson.dumps(order))
            await asyncio.wait_for(task, HANDLER_TIME_OUT)
            await sub_client.punsubscribe()
            await sub_client.close()
